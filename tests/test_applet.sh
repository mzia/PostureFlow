#!/usr/bin/env bash
# ==============================================================================
# Integration Test for postureflow-applet & StatusNotifierItem/DBusMenu
# ==============================================================================
...
echo -e "\n${BOLD}${CYAN}=== Testing postureflow-applet Integration ===${NC}"

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
DAEMON_BIN="$SCRIPT_DIR/../target/release/postureflow-daemon"
APPLET_BIN="$SCRIPT_DIR/../target/release/postureflow-applet"
...
DAEMON="./target/release/postureflow-daemon"
...
echo "[*] Step 1: Starting postureflow-daemon on test bus..."
$DAEMON --daemon --session-bus &
DAEMON_PID=$!

sleep 1

echo "[*] Step 2: Starting postureflow-applet on test bus..."
$APPLET --session-bus &
APPLET_PID=$!
...
CYCLED_PROFILE=$(gdbus call --session --dest io.github.mzia.PostureFlow --object-path /io/github/mzia/PostureFlow --method io.github.mzia.PostureFlow.GetActiveProfile)
...
TRAVEL_PROFILE=$(gdbus call --session --dest io.github.mzia.PostureFlow --object-path /io/github/mzia/PostureFlow --method io.github.mzia.PostureFlow.GetActiveProfile)
...
RESET_PROFILE=$(gdbus call --session --dest io.github.mzia.PostureFlow --object-path /io/github/mzia/PostureFlow --method io.github.mzia.PostureFlow.GetActiveProfile)
