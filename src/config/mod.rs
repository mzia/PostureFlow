...
pub const SYSTEM_PROFILES_DIR: &str = "/etc/pop-profile/profiles.d";
...
pub fn load_all_profiles() -> Vec<ProfileConfig> {
    ...
    // 1. Scan System directory (/etc/pop-profile/profiles.d)
    scan_directory(Path::new(SYSTEM_PROFILES_DIR), &mut profiles, &mut ids);

    // 2. Scan User directory (~/.config/pop-profile/profiles.d)
